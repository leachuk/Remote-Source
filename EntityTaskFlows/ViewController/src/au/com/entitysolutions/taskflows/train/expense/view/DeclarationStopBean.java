...
public class DeclarationStopBean extends TaskflowTrainStopBean {
  ...
  public boolean validate() {
    ...
    while(row!=null){
      ...
      String formFilePath=(String)row.getAttribute("FormFilePath");
      long expenseLongId=expenseId.longValue();
    
      if(!StringUtils.isEmpty(itemType)){
        if(StringUtils.isEmpty(formFilePath) && (!this.uploadFileMap.containsKey(expenseLongId) || !this.uploadFileStreamMap.containsKey(expenseLongId))){        
        context.addMessage(null,new FacesMessage(FacesMessage.SEVERITY_ERROR, "Form Upload",
                             (String)FacesUtil.resolveExpression("#{pageFlowScope.msgs.bundle['FILE_FORM_UPLOAD_ERROR']}")));               
        return false;
        }        
      }     
      row=iterator.next();
    ...
    while(row!=null){      
      ...
      if(!StringUtils.isEmpty(itemType)){       
         if(this.uploadFileMap.containsKey(expenseLongId) && this.uploadFileStreamMap.containsKey(expenseLongId)){        
          ...
          row.setAttribute("FormFilePath",(tmpfile==null)? null : tmpfile.getAbsoluteFile());                
        }           
      }     
      row=iterator.next();
    }    
  ...
  public String getStepName() {
    return (String)FacesUtil.resolveExpression("#{pageFlowScope.msgs.bundle['DECLARATION_TRAIN_STEP_NAME']}");
  }
